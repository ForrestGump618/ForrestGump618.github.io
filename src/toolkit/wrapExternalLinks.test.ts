...
describe("wrapExternalLinks", () => {
  ...
  it("为外链添加 target/rel/exturl 与图标", () => {
    ...
    const out = wrapExternalLinks(html, { siteOrigin });
  ...
  it("站内绝对 URL 不处理", () => {
    ...
    expect(wrapExternalLinks(html, { siteOrigin })).toBe(html);
  });
  ...
  it("mailto/tel 协议不处理", () => {
    ...
    expect(wrapExternalLinks(mail, { siteOrigin })).toBe(mail);
    expect(wrapExternalLinks(tel, { siteOrigin })).toBe(tel);
  });
  ...
  it("无 siteOrigin 时所有 http(s) 绝对 URL 视为外链", () => {
  ...
  it("siteOrigin 末尾斜杠被规范化", () => {
    const html = `<a href="https://example.com/x">x</a>`;
    expect(wrapExternalLinks(html, { siteOrigin: "https://example.com/" })).toBe(html);
  });
});
